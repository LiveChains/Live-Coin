...
QString QRCodeDialog::getURI()
{
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non LVC unit input in lnReqAmount, we generate the URI with LVC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::LVC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
